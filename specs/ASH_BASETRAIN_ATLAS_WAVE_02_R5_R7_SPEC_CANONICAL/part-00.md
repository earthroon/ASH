...
> Patch ID: `ASH-BASETRAIN-ATLAS-WAVE-02-R5-R7`  
> Direct physical parent: `ASH-BASETRAIN-ATLAS-WAVE-02-R5-R6`  
> Imported physical ancestors: `R5-R3`, `R5-R4`, `R5-R5`, `R5-R6`  
> Patch class: production-candidate selected-layer real-checkpoint forward closure  
> New selected-forward SSOT: `BaseTrainAtlasWave02R5R7SelectedLayerForwardAuthority`  
> New residency SSOT: `BaseTrainAtlasWave02R5R7ResidentTensorLeaseSet`  
> New decode SSOT: `BaseTrainAtlasWave02R5R7TensorDecodeAuthority`  
> Headwise output authority: forbidden  
> TensorCube output authority: forbidden  
> Full-model authority: forbidden  
...
> Rev.2 amendment: the 48,259-row vocabulary embedding must be loaded through an Atlas Parallel Streaming Wave path. Linear whole-tensor host materialization and serial monolithic upload are non-admissible.

`ASH-BASETRAIN-ATLAS-WAVE-02-R5-R7` is the first patch in the Atlas Wave chain that consumes real production-candidate checkpoint payload bytes and executes a real selected-layer attention forward surface on the GPU.

R5-R6 proved that the full 48,259-vocabulary TinyLlama-derived checkpoint is one complete, immutable, canonical tensor set. R5-R7 must now prove that five exact physical tensors from that set can be:

```text
identified from R5-R6 authority
  -> read only from their sealed shard and byte range
  -> decoded from BF16 or F16 without semantic drift
  -> uploaded into one same-device resident lease set
