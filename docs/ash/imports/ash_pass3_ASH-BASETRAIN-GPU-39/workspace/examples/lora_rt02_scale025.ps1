...
$Req = @{
  id = "lora_rt02_scale025_greeting_01"
  cmd = "run.start"
  payload = @{
    kind = "infer"
    input = $PromptText
    ...
    domainAdapterAttachReceiptPath = ".\workspace\lora_rt02_scale025_attach_receipt.json"
    domainAdapterApplyReceiptPath = ".\workspace\lora_rt02_scale025_apply_receipt.json"
    domainAdapterDeltaTracePath = ".\workspace\lora_rt02_scale025_delta_trace.json"
    domainAdapterStrictDeltaGuard = $true
    domainAdapterMaxDeltaToBaseRatio = 1.0
    eosRestoreEnabled = $true
    memoryBudgetEnabled = $true
    memoryBudgetReceiptPath = ".\workspace\lora_rt02_scale025_mem_receipt.json"
  }
} | ConvertTo-Json -Depth 30 -Compress

$Req | .\target\debug\orchestrator_local.exe 2>&1 |
  Tee-Object .\workspace\lora_rt02_scale025_greeting_01.log
